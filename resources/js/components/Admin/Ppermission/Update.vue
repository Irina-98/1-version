<template>
    <v-app>
        ...
        <div class="container mt-5">
            <form @submit.prevent="create" v-if="item.prole.portal_id != null">
                        <div class="row">
                            <div class="col-12 mb-2">
                                <div class="form-group">
                                    <label>{{$t('portal')}}</label>
                                    <select class="form-control" v-model="item.prole.portal_id" @change="portalChange()">
                            ...
                            <div class="col-12 mb-2">
                                <div class="form-group">
                                    <label>{{$t('role')}}</label>
                                    <select class="form-control" v-model="item.prole_id">
                                        <option disabled value="">{{$t('choose_one_item')}}</option>
                                        <option v-for="it in proles" :key="it.id" :value="it.id">
                                            {{it.name}}
                                        </option>
                                    </select>
                                </div>
    ...
    export default {
        ...
        methods:{
            show: function() {
                ...
                axios.post(`/api/admin/ppermission/get/${this.$route.params.id}`).then(function(response) {
                    let rs = [];
            ...
            portalChange() {
                axios.post(`/api/admin/prole/list_by_portal/${this.item.prole.portal_id}`).then(function(response){
            ...
            async update() {
                if (
                    ...
                    axios.post(`/api/admin/ppermission/update/${this.$route.params.id}`, this.item).then(function(response){
                        this.$router.push({name:"AdminPpermissionList"})
                    }.bind(this));
                }
            },
        },
