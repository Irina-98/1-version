<template>
    <v-app>
        ...
        <div class="container mt-5">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 mb-2">
                                <div class="form-group">
                                    <label>{{$t('role')}}</label>
                                    <v-select
                                        multiple class="form-control" v-model="user.role"
                                        v-on:change="roleChanged"
                                        clearable
                                        :label="$t('choose_items')"
                                        :items="roles"
                                        item-value="value"
                                        item-text="name"
                                    >
                            ...
                            <div class="col-12 mb-2">
                                <div class="form-group">
                                    ...
                                    <v-select 
                                        ...
                                        :items="lessons"
                                        :label="$t('subject_intresting_third')"
                                        :item-text="`${'name_' + $i18n.locale}`"
                                        item-value="id"
                                    >
                                    </v-select>
                    ...
                    <v-list shaped>
                        ...
                        >
                            <template v-for="(item, i) in user.portals">
                                ...
                                <template v-for="(prole) in item.proles">
                            <v-list-item
                                :key="prole.id"
                                :value="prole"
                                active-class="deep-purple--text text--accent-4"
            ...
            <div class="row">
                <div class="col-12">
                    <button type="button" class="btn btn-primary my_btn_fonts" @click="create()">{{$t('save')}}</button>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.roles = [
                {value :'admin', name: this.$t('admin')},
                {value :'employee', name: this.$t('employee')},
                {value :'parent', name: this.$t('parent')},
                {value :'student', name: this.$t('student')},
                {value :'enrollee', name: this.$t('enrollee')},
            ]
        },
        data(){
            return {
        ...
        methods:{
            async create() {
                if (
                    ...
                    axios.post('/api/admin/user/create', fd, config).then(function(response){
            ...
            roleChanged(r) {
                this.user.portals = [];
                for(let role_i in this.user.role) {
                    for(let dpr_i in this.default_portal_roles) {
                        if (this.user.role[role_i] == dpr_i) {
                            for(let portals_i in this.default_portal_roles[dpr_i]['portals']) {
                                for(let p_i in this.portals) {
                                    if (this.portals[p_i].id == this.default_portal_roles[dpr_i]['portals'][portals_i]) {
                                        let isNew = true;
                                        for(let pu_i in this.user.portals) {
                ...
                for(let role_i in this.user.role) {
                    for(let dpr_i in this.default_portal_roles) {
                        if (this.user.role[role_i] == dpr_i) {
                            for(let proles_i in this.default_portal_roles[dpr_i]['proles']) {
                                for(let p_i in this.user.portals) {
                                    for(let pr_i in this.user.portals[p_i]['proles']) {
                                        if (this.user.portals[p_i]['proles'][pr_i].id == this.default_portal_roles[dpr_i]['proles'][proles_i]) {
                                            let isNew = true;
                                            for(let pu_i in this.user.proles) {
                                                if (this.user.proles[pu_i].id == this.user.portals[p_i]['proles'][pr_i].id) {
                                                    isNew = false;
                                                }
                                            }
                                            if (isNew) {
                                                this.user.proles.push(this.user.portals[p_i]['proles'][pr_i]);
                                            }
                                        }
                                    }
                                }
                ...
                for(let role_i in this.user.role) {
                    for(let dpr_i in this.default_portal_roles) {
                        if (this.user.role[role_i] == dpr_i) {
                            for(let ppermission_i in this.default_portal_roles[dpr_i]['ppermissions']) {
                                for(let p_i in this.user.portals) {
                                    for(let pr_i in this.user.portals[p_i]['proles']) {
                                        for(let pp_i in this.user.portals[p_i]['proles'][pr_i]['ppermissions']) {
                                            if (this.user.portals[p_i]['proles'][pr_i]['ppermissions'][pp_i].id == this.default_portal_roles[dpr_i]['ppermissions'][ppermission_i]) {
                                                let isNew = true;
                                                for(let pu_i in this.user.ppermissions) {
                                                    if (this.user.ppermissions[pu_i].id == this.user.portals[p_i]['proles'][pr_i]['ppermissions'][pp_i].id) {
                                                        isNew = false;
                                                    }
                                                }
                                                if (isNew) {
                                                    this.user.ppermissions.push(this.user.portals[p_i]['proles'][pr_i]['ppermissions'][pp_i]);
                                                }
                                            }
                                        }
                                    }
